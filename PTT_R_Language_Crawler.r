...
lastpage <- unlist(xpathSApply(htmlParse( getURL(paste0("https://www.ptt.cc/bbs/R_Language/index.html"), cainfo = signatures)),  "//div[@class='btn-group btn-group-paging']/a",xmlGetAttr, "href"))[[2]]
...
for( i in (lastpage-99):lastpage){
  url <- paste0("https://www.ptt.cc/bbs/R_Language/index", i, ".html")
  html <- htmlParse(getURL(url, cainfo = signatures))
  url.list <- xpathSApply(html, "//div[@class='title']/a[@href]", xmlAttrs)
  link.R <- c(link.R, paste('https://www.ptt.cc', url.list, sep=''))
  print(paste("Get url from the billboard's(R Language) page :", i))
}
...
system.time(sapply(1:10, function(i) getdoc(link.R[i], path="C:/RDATA/PTT/R_Language/")))
